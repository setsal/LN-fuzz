...
/*
   LN-fuzz - based on american fuzzy lop - fuzzer code
   --------------------------------
...
u32 cur_fuzz_is_redundant_flag = 0;
      ...
      case FAVOR:
        if (state->seeds_count > 10) {
          ...
          while (passed_cycles < 5) {
            ...
            // small probility to skip the redundant seed
            if ( result->redundant == 1 && UR(100) < 75 ) continue;
...
int get_if_sequence_interesting(struct queue_entry *q)
{
  ...
  // check first if mutate sequence has any new state
  for ( i=0; i<state_count; i++) {
  ...
  // give a small probability to store small sequence
  if ( UR(100) < 20 ) {
    if ( abs(q->region_count - state_count) > 2 ) {
      return 0;
    }
  }
...
// setsal :not execute any calibrate_case except dry_run
static u8 calibrate_case(char** argv, struct queue_entry* q, u8* use_mem,
  ...
  if (!dumb_mode && first_run && !fault ) fault = FAULT_NOBITS;
...
#define SP20    SP10 SP10
  ...
  if (getenv("AFLNET_DEBUG") && (atoi(getenv("AFLNET_DEBUG")) == 1) && state_aware_mode) {
    ...
    SAYF(cRST "\nTarget_state_id: %-4s, Cur_queue_first_id: %-4s, Cur_queue_last_id: %-4s\n\n", DI(target_state_id), DI(cur_queue_first_id), DI(cur_queue_last_id));
...
int main(int argc, char** argv) {
  ...
  SAYF(cCYA "LN-fuzz " cBRI VERSION cRST " by <contact@setsal.dev>\n");
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:u:b:t:T:dnCB:S:M:x:QN:D:W:w:P:KEq:s:RFc:z")) > 0)
  ...
  if (state_aware_mode) {
    while (1) {
      ...
      if (cur_fuzz_is_redundant_flag == 1) {
        selected_seed->redundant = 1;
      }
