...
if $(strstr $FUZZER "afl") || $(strstr $FUZZER "ln"); then
  ...
  if [ $FUZZER = "lnfuzz" ]; then
    timeout -k 0 $TIMEOUT /home/ubuntu/${FUZZER}/src/ln-fuzz -d -i ${WORKDIR}/in-dicom -o $OUTDIR -N tcp://127.0.0.1/5158 $OPTIONS ./dcmqrscp
  else
  ...
  if [ $FUZZER = "aflnwe" ]; then
    cov_script_showmap_dcmtk ${WORKDIR}/dcmtk/build/bin/${OUTDIR}/ 5158 ${SKIPCOUNT} 0
  else
    cov_script_showmap_dcmtk ${WORKDIR}/dcmtk/build/bin/${OUTDIR}/ 5158 ${SKIPCOUNT} 1
  fi
