...
if $(strstr $FUZZER "afl") || $(strstr $FUZZER "ln"); then
  ...
  if [ $FUZZER = "lnfuzz" ]; then
    timeout -k 0 $TIMEOUT /home/ubuntu/${FUZZER}/src/ln-fuzz -d -i ${WORKDIR}/in-tls -x ${WORKDIR}/tls.dict -o $OUTDIR -N tcp://127.0.0.1/4433 $OPTIONS ./apps/openssl s_server -key key.pem -cert cert.pem -4 -naccept 1 -no_anti_replay
  else
  ...
  if [ $FUZZER = "aflnwe" ]; then
    cov_script_showmap_openssl ${WORKDIR}/openssl/${OUTDIR}/ 4433 ${SKIPCOUNT} 0
  else
    cov_script_showmap_openssl ${WORKDIR}/openssl/${OUTDIR}/ 4433 ${SKIPCOUNT} 1
  fi
