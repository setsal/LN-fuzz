...
if $(strstr $FUZZER "afl") || $(strstr $FUZZER "ln"); then
  ...
  if [ $FUZZER == "aflnwe" ]; then
    cov_script ${WORKDIR}/openssl/${OUTDIR}/ 4433 ${SKIPCOUNT} ${WORKDIR}/openssl/${OUTDIR}/cov_over_time.csv 0
  else
    cov_script ${WORKDIR}/openssl/${OUTDIR}/ 4433 ${SKIPCOUNT} ${WORKDIR}/openssl/${OUTDIR}/cov_over_time.csv 1
  fi

  gcovr -r . --html --html-details -o index.html
  mkdir ${WORKDIR}/openssl/${OUTDIR}/cov_html/
  cp *.html ${WORKDIR}/openssl/${OUTDIR}/cov_html/
