...
if $(strstr $FUZZER "afl") || $(strstr $FUZZER "ln"); then
  ...
  if [ $FUZZER = "lnfuzz" ]; then
    timeout -k 0 $TIMEOUT /home/ubuntu/${FUZZER}/src/ln-fuzz -d -i ${WORKDIR}/in-smtp -x ${WORKDIR}/smtp.dict -o $OUTDIR -N tcp://127.0.0.1/25 $OPTIONS exim -bd -oX 25
  else
  ...
  if [ $FUZZER = "aflnwe" ]; then
    cov_script_showmap_exim ${WORKDIR}/exim/${OUTDIR}/ 25 ${SKIPCOUNT} 0
  else
    cov_script_showmap_exim ${WORKDIR}/exim/${OUTDIR}/ 25 ${SKIPCOUNT} 1
  fi
