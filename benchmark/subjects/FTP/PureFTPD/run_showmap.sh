...
if $(strstr $FUZZER "afl") || $(strstr $FUZZER "ln"); then
  ...
  if [ $FUZZER = "lnfuzz" ]; then
    timeout -k 0 $TIMEOUT /home/ubuntu/${FUZZER}/src/ln-fuzz -d -i ${WORKDIR}/in-ftp -o $OUTDIR -x ${WORKDIR}/ftp.dict -N tcp://127.0.0.1/21 $OPTIONS src/pure-ftpd -A
  else
  ...
  if [ $FUZZER = "aflnwe" ]; then
    cov_script_showmap_pureftpd ${WORKDIR}/pure-ftpd/${OUTDIR}/ 21 ${SKIPCOUNT} 0
  else
    cov_script_showmap_pureftpd ${WORKDIR}/pure-ftpd/${OUTDIR}/ 21 ${SKIPCOUNT} 1
  fi
