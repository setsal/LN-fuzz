...
if $(strstr $FUZZER "afl") || $(strstr $FUZZER "ln"); then
  ...
  if [ $FUZZER = "aflnwe" ]; then
    cov_script ${WORKDIR}/bftpd/${OUTDIR}/ 21 ${SKIPCOUNT} ${WORKDIR}/bftpd/${OUTDIR}/cov_over_time.csv 0
  else
    cov_script ${WORKDIR}/bftpd/${OUTDIR}/ 21 ${SKIPCOUNT} ${WORKDIR}/bftpd/${OUTDIR}/cov_over_time.csv 1
  fi

  gcovr -r . --html --html-details -o index.html
  mkdir ${WORKDIR}/bftpd/${OUTDIR}/cov_html/
  cp *.html ${WORKDIR}/bftpd/${OUTDIR}/cov_html/
