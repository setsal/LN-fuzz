...
if $(strstr $FUZZER "afl") || $(strstr $FUZZER "ln"); then
  ...
  if [ $FUZZER = "lnfuzz" ]; then
    timeout -k 0 $TIMEOUT /home/ubuntu/${FUZZER}/src/ln-fuzz -d -i ${WORKDIR}/in-ftp -o $OUTDIR -x ${WORKDIR}/ftp.dict -N tcp://127.0.0.1/21 $OPTIONS bftpd -D -c ${WORKDIR}/basic.conf
  else
    timeout -k 0 $TIMEOUT /home/ubuntu/${FUZZER}/afl-fuzz -d -i ${WORKDIR}/in-ftp -o $OUTDIR -x ${WORKDIR}/ftp.dict -N tcp://127.0.0.1/21 $OPTIONS bftpd -D -c ${WORKDIR}/basic.conf
  fi
  ...
  if [ $FUZZER = "aflnwe" ]; then
    cov_script_showmap_bftpd ${WORKDIR}/bftpd/${OUTDIR}/ 21 ${SKIPCOUNT} 0
  else
    cov_script_showmap_bftpd ${WORKDIR}/bftpd/${OUTDIR}/ 21 ${SKIPCOUNT} 1
  fi
