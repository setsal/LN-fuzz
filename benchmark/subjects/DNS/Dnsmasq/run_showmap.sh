...
if $(strstr $FUZZER "afl") || $(strstr $FUZZER "ln"); then
  ...
  if [ $FUZZER = "lnfuzz" ]; then
    timeout -k 0 $TIMEOUT /home/ubuntu/${FUZZER}/src/ln-fuzz -d -i ${WORKDIR}/in-dns -o $OUTDIR -N tcp://127.0.0.1/5353 $OPTIONS ./dnsmasq
  else
  ...
  if [ $FUZZER = "aflnwe" ]; then
    cov_script_showmap_dnsmasq ${WORKDIR}/dnsmasq/src/${OUTDIR}/ 5353 ${SKIPCOUNT} 0
  else
    cov_script_showmap_dnsmasq ${WORKDIR}/dnsmasq/src/${OUTDIR}/ 5353 ${SKIPCOUNT} 1
  fi  
