...
if $(strstr $FUZZER "afl") || $(strstr $FUZZER "ln"); then
  ...
  if [ $FUZZER = "lnfuzz" ]; then
    timeout -k 0 $TIMEOUT /home/ubuntu/${FUZZER}/src/ln-fuzz -d -i ${WORKDIR}/in-ssh -x ${WORKDIR}/ssh.dict -o $OUTDIR -N tcp://127.0.0.1/22 $OPTIONS ./sshd -d -e -p 22 -r -f sshd_config
  else
  ...
  if [ $FUZZER = "aflnwe" ]; then
    cov_script_showmap_openssh ${WORKDIR}/openssh/${OUTDIR}/ 22 ${SKIPCOUNT} 0
  else
    cov_script_showmap_openssh ${WORKDIR}/openssh/${OUTDIR}/ 22 ${SKIPCOUNT} 1
  fi
