...
if $(strstr $FUZZER "afl") || $(strstr $FUZZER "ln"); then
  ...
  if [ $FUZZER = "lnfuzz" ]; then
    timeout -k 0 $TIMEOUT /home/ubuntu/${FUZZER}/src/ln-fuzz -d -i ${WORKDIR}/in-dtls -o $OUTDIR -N udp://127.0.0.1/20220 $OPTIONS ./tinydtls/tests/dtls-server
  else
  ...
  if [ $FUZZER == "aflnwe" ]; then
    cov_script_showmap_tinydlts ${WORKDIR}/${OUTDIR}/ 20220 ${SKIPCOUNT} 0
  else
    cov_script_showmap_tinydlts ${WORKDIR}/${OUTDIR}/ 20220 ${SKIPCOUNT} 1
  fi
